...
export class AISchedulerService {
    async generateOptimalSchedule() {
        ...
        const openTables = tables.filter((t) => t.status === TableStatus.Open && !t.isLocked);
        const newAssignments = [];
        let dealerIndex = 0;
        for (const table of openTables) {
            if (dealerIndex >= availableDealers.length)
                break;
            const dealer = availableDealers[dealerIndex];
            const assignment = {
                id: Math.max(0, ...assignments.map((a) => a.id)) + newAssignments.length + 1,
                dealerId: dealer.id,
                tableId: table.id,
                startTime: new Date(),
                endTime: null,
            ...
            dealers[dealers.findIndex((d) => d.id === dealer.id)].status = DealerStatus.Dealing;
            dealerIndex++;
        }
    ...
    async autoRotate() {
        ...
        const assignments = db.tables.get('Assignments') || [];
        const dealers = db.tables.get('Dealers') || [];
        const activeAssignments = assignments.filter((a) => a.isCurrent && !a.endTime);
        let rotated = 0;
        for (const assignment of activeAssignments) {
            ...
            if (minutesElapsed >= 20) {
                ...
                const dealerIndex = dealers.findIndex((d) => d.id === assignment.dealerId);
                if (dealerIndex !== -1) {
